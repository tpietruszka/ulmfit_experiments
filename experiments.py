...
class ExperimentCls(metaclass=RegisteredAbstractMeta, is_registry=True):
    ...
    # one way of splitting data - train and validation are generated by CV (not touching test in this setup)
    cv_num_splits: int = 5
    cv_random_state: int = 17
    cv_fold_num: int = 0

    # options for choosing different train/val/test splits
    new_split: bool = False  # this disables "train_set_fraction" setting
    new_split_train_fraction: float = 0.5
    ...
    def get_dfs(self, fold_num: Optional[int] = 0) -> [DataFrame, DataFrame, DataFrame]:
        if fold_num is None:
            fold_num = self.cv_fold_num
        dfs = self.load_dfs(fold_num)
        if self.new_split:
    ...
    @abstractmethod
    def load_dfs(self, fold_num: int) -> [DataFrame, DataFrame, DataFrame]:
        """fold_num passed by argument to support some legacy notebooks"""
        pass
